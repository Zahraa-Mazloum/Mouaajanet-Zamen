// src/components/Navbar.tsx
'use client'

import { useState } from 'react'
import Link from 'next/link'
import Image from 'next/image'
import { usePathname } from 'next/navigation'
import { useTranslations, useLocale } from 'next-intl'
import ClientToggles from '@/components/ClientToggles'

export default function Navbar() {
  const [isOpen, setIsOpen] = useState<boolean>(false)
  const pathname = usePathname()
  const t = useTranslations('nav')
  const locale = useLocale()

  const links = [
    { name: t('home'),     path: `/${locale}` },
    { name: t('products'), path: `/${locale}/products` },
    { name: t('contact'),  path: `/${locale}/contact` },
  ]

  function isActive(path: string): boolean {
    return pathname === path || pathname === path + '/'
  }

  return (
    <header style={{
      background: 'var(--navbar-bg)',
      borderBottom: '1px solid var(--navbar-border)'
    }}>
      <nav className="mx-auto flex justify-between items-center w-[95%] py-2">

        {/* ── Logo ─────────────────────────────────── */}
        <div className="z-50">
          <Link href={`/${locale}`}>
            <Image
              src="/logos/Bakery_LogoTrans.png"
              alt="Bakery Logo"
              width={160}
              height={160}
              loading="eager"
            />
          </Link>
        </div>

        {/* ── Nav Links ────────────────────────────── */}
        <div
          className={`md:static md:min-h-fit absolute min-h-full w-full z-20
            ${isOpen ? 'top-[17%]' : 'top-[-100%]'} expand-navbar`}
          style={{ background: isOpen ? 'var(--navbar-bg)' : 'transparent' }}
        >
          <ul>
            {links.map((link) => (
              <Link
                key={link.path}
                href={link.path}
                onClick={() => setIsOpen(false)}
                className="relative group inline-block hover:text-(--primarybutton) transition px-3 py-1"
              >
                {link.name}
                <svg
                  viewBox="0 0 66 39"
                  preserveAspectRatio="none"
                  className="absolute inset-0 w-full h-full pointer-events-none md:block hidden"
                >
                  <path
                    d="M47.6933 8.97338C32.2599 6.35381 10.1095 6.48289 3.63879 18.8108C-1.00186 27.6521 1.72618 41.409 45.3177 35.6217C77.4153 31.3603 63.378 2 38.9883 2"
                    stroke="currentColor"
                    strokeWidth="3"
                    fill="none"
                    className={`scribble-path ${isActive(link.path) ? 'scribble-active' : ''}`}
                  />
                </svg>
              </Link>
            ))}

            {/* ── Mobile extras ── */}
            <li className="flex flex-col items-center gap-4 md:hidden list-none">
              <Link href={`/${locale}/wishlist`} className="hover:text-(--hovercolor) transition">
                {t('wishlist')}
              </Link>
              <Link href={`/${locale}/cart`} className="hover:text-(--hovercolor) transition">
                {t('cart')}
              </Link>

              <div className="flex items-center bg-(--bgcolor) rounded-full px-6 py-3">
                <SearchIcon />
                <input
                  type="text"
                  placeholder={t('search')}
                  className="ml-2 bg-transparent focus:outline-none"
                />
              </div>

              {/* Mobile toggles */}
              <ClientToggles locale={locale} currentPath={pathname} />

              <button
                type="button"
                id="primaryBtn"
                onClick={() => window.location.href = `/${locale}/login`}
              >
                {t('login')}
              </button>
            </li>
          </ul>
        </div>

        {/* ── Right side — desktop ─────────────────── */}
        <div className="flex items-center gap-3">

          {/* Search — desktop */}
          <div className="hidden md:flex items-center bg-(--bgcolor) rounded-full px-6 py-3">
            <SearchIcon />
            <input
              type="text"
              placeholder={t('search')}
              className="mx-2 bg-transparent focus:outline-none w-32"
            />
          </div>

          {/* Wishlist */}
          <Link href={`/${locale}/wishlist`} className="hidden md:block hover:text-(--hovercolor) transition">
            <WishlistIcon />
          </Link>

          {/* Cart */}
          <Link href={`/${locale}/cart`} className="hidden md:block hover:text-(--hovercolor) transition">
            <CartIcon />
          </Link>

          {/*  ALL three toggles live here — isolated, no hydration issues */}
          <div className="hidden md:flex">
            <ClientToggles locale={locale} currentPath={pathname} />
          </div>

          {/* Hamburger */}
          <button
            type="button"
            onClick={() => setIsOpen(prev => !prev)}
            className="md:hidden cursor-pointer"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
              viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-8">
              {isOpen
                ? <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                : <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
              }
            </svg>
          </button>

          {/* Login */}
          <button
            type="button"
            id="primaryBtn"
            className="hover:bg-(--buttonhover) hover:text-white hidden md:flex"
            onClick={() => window.location.href = `/${locale}/login`}
          >
            {t('login')}
          </button>

        </div>
      </nav>
    </header>
  )
}

// ── Icons ─────────────────────────────────────────────────
function SearchIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-5">
      <path strokeLinecap="round" strokeLinejoin="round"
        d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
    </svg>
  )
}

function WishlistIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
      <path strokeLinecap="round" strokeLinejoin="round"
        d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
    </svg>
  )
}

function CartIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
      viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
      <path strokeLinecap="round" strokeLinejoin="round"
        d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
    </svg>
  )
}
